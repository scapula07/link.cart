import { AdvancedLogicTypes, CurrencyTypes, ExtensionTypes, PaymentTypes } from '@requestnetwork/types';
...
export interface IPaymentRetriever<TPaymentNetworkEvent extends PaymentTypes.IPaymentNetworkEvent<unknown, TEventNames>, TEventNames = PaymentTypes.EVENTS_NAMES> {
    getTransferEvents(): Promise<TPaymentNetworkEvent[]>;
}
export interface ITheGraphBaseInfoRetriever<TPaymentEvent extends PaymentTypes.IETHPaymentEventParameters, TTransferEventsParams extends TransferEventsParams = TransferEventsParams> {
    getTransferEvents(params: TTransferEventsParams): Promise<PaymentTypes.AllNetworkEvents<TPaymentEvent>>;
}
/** Generic info retriever interface without transfers */
export interface IEventRetriever<TPaymentNetworkEvent extends PaymentTypes.IPaymentNetworkBaseEvent<TEventNames>, TEventNames = PaymentTypes.EVENTS_NAMES> {
    getContractEvents(): Promise<TPaymentNetworkEvent[]>;
}
/** Object interface to list the payment network module by id */
export type IPaymentNetworkModuleByType<TPaymentEventParameters extends PaymentTypes.GenericEventParameters = PaymentTypes.GenericEventParameters> = Partial<Record<ExtensionTypes.PAYMENT_NETWORK_ID, new (...pnParams: any) => PaymentDetectorBase<ExtensionTypes.IExtension, TPaymentEventParameters>>>;
/** Object interface to list the payment network module by network */
export interface ISupportedPaymentNetworkByNetwork<TPaymentEventParameters extends PaymentTypes.GenericEventParameters = PaymentTypes.GenericEventParameters> {
...
export interface ISupportedPaymentNetworkByCurrency<TPaymentEventParameters extends PaymentTypes.GenericEventParameters = PaymentTypes.GenericEventParameters> {
    [currency: string]: ISupportedPaymentNetworkByNetwork<TPaymentEventParameters>;
}
export type TGetSubGraphClient<TChain extends CurrencyTypes.ChainName> = (network: CurrencyTypes.ChainName) => TChain extends CurrencyTypes.VMChainName ? TheGraphClient<TChain> | undefined : undefined;
export type PaymentNetworkOptions<TChain extends CurrencyTypes.ChainName = CurrencyTypes.ChainName> = {
