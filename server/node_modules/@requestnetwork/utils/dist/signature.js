...
function getIdentityFromSignatureParams(signatureParams) {
    if (signatureParams.method === types_1.SignatureTypes.METHOD.ECDSA) {
        return {
            ...
            value: crypto_1.default.EcUtils.getAddressFromPrivateKey(signatureParams.privateKey),
        };
    }
...
function sign(data, signatureParams) {
    let value;
    if (signatureParams.method === types_1.SignatureTypes.METHOD.ECDSA) {
        value = crypto_1.default.EcUtils.sign(signatureParams.privateKey, crypto_1.default.normalizeKeccak256Hash(data).value);
    ...
    if (signatureParams.method === types_1.SignatureTypes.METHOD.ECDSA_ETHEREUM) {
        const normalizedData = crypto_1.default.normalize(data);
        value = crypto_1.default.EcUtils.sign(signatureParams.privateKey, ethers_1.ethers.utils.hashMessage(normalizedData));
        return { data, signature: { method: signatureParams.method, value } };
    }
    throw new Error('signatureParams.method not supported');
}
 ...
 */
function recover(signedData) {
    let value;
    if (signedData.signature.method === types_1.SignatureTypes.METHOD.ECDSA) {
        value = crypto_1.default.EcUtils.recover(signedData.signature.value, crypto_1.default.normalizeKeccak256Hash(signedData.data).value);
        return {
            type: types_1.IdentityTypes.TYPE.ETHEREUM_ADDRESS,
            value,
        };
    ...
    if (signedData.signature.method === types_1.SignatureTypes.METHOD.ECDSA_ETHEREUM) {
        ...
        const normalizedData = ethers_1.ethers.utils.hashMessage(crypto_1.default.normalize(signedData.data));
        value = crypto_1.default.EcUtils.recover(signature, normalizedData).toLowerCase();
        return {
            type: types_1.IdentityTypes.TYPE.ETHEREUM_ADDRESS,
            value,
        };
