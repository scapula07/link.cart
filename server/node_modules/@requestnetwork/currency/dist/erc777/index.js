"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.getSupportedERC777Tokens = void 0;
const networks_1 = require("./networks");
/**
 * Returns a list of supported ERC777 tokens
 *
 * @returns List of supported ERC777 tokens
 */
function getSupportedERC777Tokens() {
    return Object.entries(networks_1.supportedNetworks).reduce((acc, [networkName, supportedCurrencies]) => {
        return [
            ...acc,
            ...Object.entries(supportedCurrencies).map(([address, token]) => ({
                address,
                network: networkName,
                decimals: token.decimals,
                symbol: token.symbol,
            })),
        ];
    }, []);
}
exports.getSupportedERC777Tokens = getSupportedERC777Tokens;
//# sourceMappingURL=index.js.map